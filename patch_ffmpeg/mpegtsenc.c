...
static MpegTSService* find_service(AVFormatContext *s, int stream_idx)
{
	int j, k, found = 0;
	MpegTSWrite *ts = s->priv_data;
	MpegTSService *sv = NULL;

	for (j = 0; j < s->nb_programs; j++){
        for (k = 0; k < s->programs[j]->nb_stream_indexes; k++)
            if (s->programs[j]->stream_index[k] == stream_idx) {
                found = 1;
                break;
            }
		if(found){
	...
	for(k = 0; k < ts->nb_services; k++){
		if(s->programs[j]->id == ts->services[k]->sid){
			sv = ts->services[k];
			break;
		}
	}
...
static int mpegts_init(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO &&
			...
			if(ts->mux_rate > 1){
				...
				// max delta PCR 0.1s TODO: should be avg_frame_rate
	...
	av_log(s, AV_LOG_VERBOSE, "muxrate %d, sdt period %d, pat period %d", 
		ts->mux_rate, ts->sdt_packet_period, ts->pat_packet_period);
